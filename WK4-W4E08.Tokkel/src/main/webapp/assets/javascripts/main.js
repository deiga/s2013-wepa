...
App.Models.Task = Backbone.Model.extend({
  urlRoot: function() {
    ...
    if (projectId) {
      return "app/projects/" + projectId + "/tasks";
    } else {
      return "app/tasks";
    }
  }
...
App.Views.TaskList = Backbone.View.extend({
  ...
  $(this.el).find("#taskAddForm").submit(function(event) {
      ...
      task.save({ description : taskDescription, projectId: selectedProjectId}, {
        ...
        error: function(model, response) {
          // FIXME: workaround for W6E06.2 where projectId is not yet available
          task.save({ description : taskDescription }, {
            success: function() {
              self.collection.add(task, {at: 0});
            },
            error: function(model, response) {
